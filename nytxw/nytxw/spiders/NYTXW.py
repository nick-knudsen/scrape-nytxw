...
function main(splash, args)
    ...
    while not splash:select('g[data-group=cells]') do
        splash:wait(0.1)
    end
...
class NytxwSpider(scrapy.Spider):
    ...
    def start_scraping(self, response):
        ...
        #url = "https://www.nytimes.com/svc/crosswords/v6/puzzle/daily/1993-11-21.json"
        yield SplashRequest(url=url,
                            ...
                            args={
                                'width': 1000,
                                'lua_source': grid_script
                            })

    def parse(self, response):
        # grid = scrapy.Request.from_curl(
        ...
        # )
        # cookies_dict = {cookie['name']: cookie['value'] for cookie in response.data['cookies']}
        # grid = SplashRequest(url='https://www.nytimes.com/svc/crosswords/v6/puzzle/daily/1993-11-21.json', cookies=cookies_dict)
        #data = json.loads(response.text)
        
        #harData = json.loads(response.text)
        #responseData = harData['har']['log']['entries']
        #responseData = harData['grid']
